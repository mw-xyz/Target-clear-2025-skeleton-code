...
def Main():
    ...
    no = -1        
    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, no)
        ...
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                ...
                if IsTarget:
                    noo = str(no)
                    if len(noo) > 1:
                        option = input("type 1 to use the first digit, 2 to use the second digit or 3 to use the whole number in your next expression: ")
                        x = 0
                        if option == '1':
                            x = noo[0]
                        elif option == '2':
                    ...
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, int(x))
        Score -= 1
        if Targets[0] != -1:
            GameOver = True
        else:
...
def FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, no):
    if TrainingGame:
        return [2, 3, 2, 8, 512]
    else:
        if no != -1:
            NumbersAllowed.append(no)
