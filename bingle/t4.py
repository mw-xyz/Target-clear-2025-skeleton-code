...
def Main():
    ...
    MaxNumber = 0
    TrainingGame = False
    Choice = input("Enter y to play the training game, \ns for standard, \ne for easy, \nm for medium, and \nh for hard: ").lower()
    print()
    if Choice == "y":
        MaxNumber = 1000
        MaxTarget = 1000
    ...
    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, Choice)
    PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber, Choice)
    input()
    
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber, Choice):
    Score = 0
    GameOver = False
    while not GameOver:
...
def FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, Choice):
    if TrainingGame:
        return [2, 3, 2, 8, 512]
    else:
        largeNos = [25, 50, 75, 100]
        if Choice == "e":
            x = 1
        elif Choice == "m":
            x = 2
            ...
            x = 0
        for i in range(0, x):
            randNo = random.choice(largeNos)
            NumbersAllowed.append(randNo)
