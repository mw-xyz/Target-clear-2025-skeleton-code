...
def Main():
    ...
    MaxNumber = 0
    TrainingGame = False
    Choice = input("Enter y to play the training game, m to load a game, anything else to play a random game: ").lower()
    print()
    if Choice == "y":
        MaxNumber = 1000
        MaxTarget = 1000
...
def RestoreGame():
    ...
    NumbersAllowed = []
    Score = 0
    str = input("Enter save game code here: ")
    while len(str) != 26:
        print("String must contain 26 characters")
        str = input("Enter save game code here: ")
    for char in str[:19]:
        if char == "@":
            Targets.append(-1)
            ...
            newchar = ord(char)
            newchar -= 64
            Targets.append(newchar)
    for char in str[20:25]:
        newchar = ord(char)
        newchar -= 64
        NumbersAllowed.append(newchar)
    newchar = ord(str[-1])
    newchar -= 64
    Score = newchar
    print(Targets)
