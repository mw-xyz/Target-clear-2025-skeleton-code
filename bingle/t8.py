...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        if x == 1:
            challengeMode = True
            print("challenge mode activated - use all numbers to score +10 points, if not, then score -5 points")
...
def CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score, challengeMode, NumbersAllowed):
    ...
    if UserInputEvaluation != -1:
        ...
        flag = True
        if challengeMode:
            for element in NumbersAllowed:
                if int(element) not in ee:
                    flag = False
                    Score -= 5
                    break
        if flag:
