...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        # Task 17 begin change
        pf = False
        pfi = input("Type y to use postfix or anything else to use infix: ").lower()
        if pfi == "y":
            pf = True
        UserInput = input("Enter an expression: ")
        print()
        if CheckIfUserInputValid(UserInput) or pf:
            UserInputInRPN = ConvertToRPN(UserInput, pf)
            # Task 17 end change (ConverttoRPN)
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                IsTarget, Score = CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score)
                if IsTarget:
                    NumbersAllowed = RemoveNumbersUsed(UserInput, MaxNumber, NumbersAllowed, pf)
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber)
        Score -= 1
        if Targets[0] != -1:
            GameOver = True
...
def RemoveNumbersUsed(UserInput, MaxNumber, NumbersAllowed, pf):
    UserInputInRPN = ConvertToRPN(UserInput, pf)
...
# Task 17 start change
def ConvertToRPN(UserInput, pf):
    if not pf:
        Position = 0
        Precedence = {"+": 2, "-": 2, "*": 4, "/": 4}
        Operators = []
