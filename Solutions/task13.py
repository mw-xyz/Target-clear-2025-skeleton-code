    ...
    while not GameOver:
        ...
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                IsTarget, Score, Targets = CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score)
                if IsTarget:
    ...
    if UserInputEvaluation != -1:
        ...
        if len(lst) == 2:
            for i in range(lst[0], lst[1]):
                Targets.remove(Targets[lst[0]])
            for element in Targets:
                if element != -1:
                    x=Targets.index(element)
                    break
            while -1 in Targets[x:]:
                    Targets.remove(-1)
    return UserInputEvaluationIsATarget, Score, Targets
...
def UpdateTargets(Targets, TrainingGame, MaxTarget):
    ...
    if TrainingGame:
        ...
        # Task 13 begin change
        t = GetTarget(MaxTarget)
