...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    Score = 0
    GameOver = False
    # Task 11 start change
    helpMode = False
    while not GameOver:
        identifiedTarget = []
        DisplayState(Targets, NumbersAllowed, Score, identifiedTarget)
        isHelp = input("Do you want to use help mode? Type y or n: ").lower()
        helpMode = isHelp == "y"
        if helpMode:
            identifiedTarget = GetRandomSuggestions(NumbersAllowed, Targets)
            DisplayState(Targets, NumbersAllowed, Score, identifiedTarget)
    # Task 11 end change (GetRandomSuggestions)
...
def GetRandomSuggestions(NumbersAllowed, Targets):
    ...
    while tries < 30:
        tries += 1
        num1 = random.choice(NumbersAllowed)
        num2 = random.choice(NumbersAllowed)     
        ...
        if op == "+":
            ...
            res = num1 / num2
        if res in Targets:
            good_targets.append(res)
        if len(good_targets) == 5:
            return good_targets
    return good_targets
# Task 11 end change (DisplayState)
...
# Task 11.1 begin change
def DisplayState(Targets, NumbersAllowed, Score, identifiedTarget):
    DisplayTargets(Targets, identifiedTarget)
    # Task 11.1 end change (DisplayTargets)
...
# Task 11.1 begin change
def DisplayTargets(Targets, identifiedTarget):
    print("|", end = '')
    for T in Targets:
        if T == -1:
            print(" ", end = '')
        elif T in identifiedTarget:
            print(f"*{T}*", end = '')
        else:
            print(T, end = '')           
        print("|", end = '')
