...
def Main():
    ...
    MaxNumber = 0
    TrainingGame = False
    # Task 15 start change
    Score = 0
    Choice = input("Enter y to play the training game, L to load a previous game state, anything else to play a random game: ").lower()
    print()
    if Choice == "y":
        MaxNumber = 1000
        MaxTarget = 1000
...
def RestoreGame():
    ...
    Score = 0
    while not valid:
        prevState = input("enter your 26 char game save code: ")
        if len(prevState) == 26 and all(ord(char) <= 127 for char in prevState):
            valid = True
        elif len(prevState) != 26:
            print("Game code is too long, must be 26 characters")
        else:
    ...
    for i in range(26):
        ...
        if char == "@":
            ...
            if numCode == "@":
                numCode = -1
            if 0 <= i <= 19:
                Targets.append(int(numCode))
            elif 20 <= i <= 24:
                NumbersAllowed.append(int(numCode))
            else:
                a = 4
                Score = numCode
