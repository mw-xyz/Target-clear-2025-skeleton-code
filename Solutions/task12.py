...
def CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score):
    ...
    txt = "was" if UserInputEvaluationIsATarget else "was not"
    if UserInputEvaluationIsATarget:
        print(f"Your expression evaluated to {UserInputEvaluation} and it {txt} a target.\nIt was found {st} times.")        
    # Task 12 end change (Check numbers used are in numbers allowed)
...
def CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
    ...
    if bad_numbers:
        print("You used the following numbers which are not in the numbers allowed.")
        print(bad_numbers)
        return False
    return True
    # Task 12 change
