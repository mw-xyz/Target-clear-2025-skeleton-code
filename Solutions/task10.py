...
# Task 10.1 begin change
class UndoState:
    def __init__(self, numbersAllowed, targets, score):
        self.numbersAllowed = deepcopy(numbersAllowed)
...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        oldState = UndoState(NumbersAllowed, Targets, Score) #
        stack = addToUndo(oldState, stack) # 
        UserInput = input(f"Enter an expression, or type U to undo.\nYou have {undosAvailable} undos available: ")
        print()
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                ...
                if IsTarget:
                    ...
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber)
        ####
        elif UserInput.upper() == "U" and undosAvailable > 0:
            NumbersAllowed, Targets, Score = undoLastTurn(stack)
            undosAvailable -= 1
        if UserInput.upper() != "U":
        ####
            Score -= 1
        if Targets[0] != -1:
            GameOver = True
...
def addToUndo(oldState, stack):
    stack.append(oldState)
    return stack

def undoLastTurn(stack):
    """returns last numsAllowed, targets, score in that order"""
    obj = stack.pop(-2)
    score = obj.get_score()
    targets = obj.get_targets()
    numsAllowed = obj.get_numbersAllowed()
    return numsAllowed, targets, score
